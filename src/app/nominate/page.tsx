              ...
              <form id="nominate-form" onSubmit={handleSubmit} className="nominate-form" noValidate>
                ...
                <fieldset className="nominate-fieldset">
                  ...
                  {cios.map((c, idx) => {
                    const subcategoryOptions: SubcategoryOption[] = subcategories
                      .filter((subcategory) => {
                        const rawCat = subcategory.categoryId;
                        let subcategoryCategoryId: string | undefined;

                        if (typeof rawCat === 'string') {
                          subcategoryCategoryId = rawCat;
                        } else if (typeof rawCat === 'object' && rawCat !== null) {
                          subcategoryCategoryId =
                            (rawCat as { id?: string; _id?: string }).id ||
                            (rawCat as { id?: string; _id?: string })._id?.toString();
                        } else {
                          subcategoryCategoryId =
                            subcategory.categoryID ?? subcategory.category?.id;
                        }

                        return String(subcategoryCategoryId) === String(c.categoryId);
                      })
                      .map((subcategory) => ({
                        value: subcategory.id,
                        label: subcategory.name ?? subcategory.label ?? '',
