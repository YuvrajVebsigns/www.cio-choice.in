...
// export default function RecognizedBrands2022Page() {
...
//       const data = await fetchWebsitePageBySlug('recognized-brands-2022');
...
//           <h1 className="rcn-title">{page?.title || 'Recognized Brands 2022'}</h1>
...
function extractItems(page: WebsitePage | null): BrandItem[] {
  ...
  function addItem(value: Record<string, unknown>) {
     ...
     * Isse page heading ka extra card create nahi hoga.
     */
    const author =
     ...
     */
    if (normalizedAuthor === pageTitle || normalizedAuthor === 'recognized brands 2022') {
      return;
    }
  ...
  function search(value: unknown) {
    ...
    Object.entries(value).forEach(([key, nestedValue]) => {
      ...
      /*
       * Nested data search hoga, lekin current object
       * directly card mein convert nahi hoga.
       */
      search(nestedValue);
    });
  }
  ...
  useEffect(() => {
    ...
    async function loadPage() {
      try {
        ...
        // console.error('Failed to load Recognized Brands 2022:', error);
  ...
  return (
    <main className="rcn-main">
      ...
      <section className="advisory-panel-section">
        <div className="advisory-container">
          {errorMessage ? (
            <p className="rcn-empty rcn-error" role="alert">
              {errorMessage}
            </p>
          ) : items.length === 0 ? (
