...
function extractItems(page: WebsitePage | null): BrandItem[] {
  ...
  function addItem(value: Record<string, unknown>) {
     ...
     * Isse page heading extra card nahi banegi.
     */
    const author =
    ...
    // Page heading ko card banne se rokta hai
    if (normalizedAuthor === pageTitle || normalizedAuthor === 'recognized brands 2024') {
      return;
    }
  ...
  function search(value: unknown) {
    ...
    Object.entries(value).forEach(([key, nestedValue]) => {
      ...
      /*
       * Nested objects search honge, lekin current
       * object ko direct card nahi banaya jayega.
       */
      search(nestedValue);
    });
  }
  ...
  useEffect(() => {
    ...
    async function loadPage() {
      try {
        ...
        // console.error('Failed to load Recognized Brands 2024:', error);
