...
export const metadata = {
  title: 'Red Carpet Night 2026',
  description:
    'Celebrate Red Carpet Night 2026 with CIO Choice recognition and curated brand honors.',
};

type RedCarpetNightPageProps = {
  params: {
    slug?: string[];
  };
};

function isValidRedCarpetNightSlug(slugParam?: string[] | string) {
  const rawValue = Array.isArray(slugParam) ? (slugParam[0] ?? '') : (slugParam ?? '');
  return rawValue === 'red-carpet-night-2026';
}

export default function RedCarpetNightPage({ params }: RedCarpetNightPageProps) {
  if (!isValidRedCarpetNightSlug(params.slug)) {
    notFound();
  }

  const year = '2026';
  const heading = `Red Carpet Night ${year}`;
    ...
    <main className="rcn-main">
      <section className="rcn-hero">
        ...
        <div className="rcn-hero-content">
          ...
          <h1 className="rcn-title">{heading}</h1>
      ...
      <section className="rcn-cards-section">
        <div className="rcn-cards-grid">
          ...
          <article className="rcn-card">
            <h2>Yearly Honors</h2>
            <p>
              The {year} edition celebrates innovation, trust, and customer excellence across the
              CIO ecosystem.
            </p>
          </article>
          <article className="rcn-card">
